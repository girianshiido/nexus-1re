(() => {
  ...
  function renderQuestionCanvases() {
    dom.questionVisual.querySelectorAll("canvas[data-plot='line']").forEach(canvas => {
      ...
      const height = Math.round(Math.max(180, Math.min(width * 0.48, window.innerHeight * 0.28)));
      ...
      for (let x = xMin; x <= xMax; x += 1) {
        ...
        if (x !== 0) { context.fillStyle = "rgba(190, 218, 225, 0.7)"; context.fillText(x < 0 ? `−${Math.abs(x)}` : String(x), px(x), py(0) + 5); }
      }
      ...
      for (let y = yMin; y <= yMax; y += 1) {
        ...
        if (y !== 0 && y % 2 === 0) { context.fillStyle = "rgba(190, 218, 225, 0.7)"; context.fillText(y < 0 ? `−${Math.abs(y)}` : String(y), px(0) - 5, py(y)); }
      }
